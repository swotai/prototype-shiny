...
shinyServer(
  function(input,output) {
    ...
    plotdata <- reactive({
      ...
      thisData$cost  <- exp(thisData$lcost)*thisData$length + thisData$c_mc/1000000
      thisData$costpm <- thisData$cost/thisData$length
      thisData$lifetime_b <- thisData$lifetime_b/1000000  # Scale it to the millions
      
      # Recalculate MC and MB
      thisData$mc   <- append(diff(thisData$cost),NA,0)
      thisData$mb   <- append(diff(thisData$lifetime_b),NA,0)
    ...
    output$MBMC <- renderPlot({
      ...
      p <- ggplot(data=plotdata(), aes_string(x="speed")) +
        geom_ribbon(aes_string(ymax="mb",ymin="mc"))
      print(p)
    })#, width=800)
    ...
    output$datatable <- renderTable({
      plotdata()[c('speed','llength','lpopden','lempden','lcost','cost','mc','lifetime_b','mb')][1:12,]
    })
  }
)
